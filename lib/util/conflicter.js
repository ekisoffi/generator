...
class Conflicter {
  ...
  _ask(file, cb) {
    ...
    if (fs.statSync(file.path).isFile()) {
      ...
      if (!binaryDiff.isBinary(file.path, file.contents)) {
        prompt.choices.push({
          key: 'm',
          name: 'Launch an external merge tool',
          value: 'merge'
        });
      }
    }
  ...
  _mergeProcess(file) {
    const tmpfile = this._temporaryFile(file);
    try {
      if (shell.exec(this._mergeTool(tmpfile, file.path), {silent: true}).code !== 0) {
        this.adapter.log.writeln('Error with mergetool');
        return 1;
      }
    } finally {
      shell.rm(tmpfile);
  ...
  _mergeTool(fn1, fn2) {
    const args = ' ' + fn1 + ' ' + fn2;
    return 'ediff.sh' + args;
  }

  _temporaryFile(file) {
    const fname = path.resolve('/tmp', uuidv4());

    fs.writeFileSync(fname, file.contents, function (err) {
      if (err) {
        this.adapter.log.writeln('Aborting ... ' + err);
        throw new AbortedError();
      }
    });
